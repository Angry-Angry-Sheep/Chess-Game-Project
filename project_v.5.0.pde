...
void draw() {
  ...
  // --- draw tiles (but not the pieces yet) ---
  for (int i = 0; i < cols; i++) {
    for (int j = 0; j < rows; j++) {
      tiles[i][j].update();
      tiles[i][j].display();
...
void mousePressed() {
  ...
  if (isHighlighted) {
    ...
    // clear UI and run dat tobis AI move immediately
    deselectAll();

    runTobisAIMove();   // <<< engine plays here VERY IMPORTANT DONT FORGET!!!!!
...
boolean isKingAlive(String side) {
  for (int c = 0; c < cols; c++) {
    for (int r = 0; r < rows; r++) {
      Piece p = pieces[c][r];
      if (p != null && p.side.equals(side) && p.type.equals("king")) return true;
    }
  }
  return false;
}
...
class Piece {
  ...
  void display() {
    ...
    image(img, x + (tileSize - w)/2 - offset, y + (tileSize - h)/2 - offset, w, h);
  }
}
...
void runTobisAIMove() {
  if (engine == null || aiThinking) return; // dont start move twice

  aiThinking = true;
  aiResult = null;

  // start computeTobisMove() in another thread so that it wont lag
  ...
  for (int c = 0; c < cols; c++) {
    for (int r = 0; r < rows; r++) {
      ...
      for (Cell mv : leg) {
        ...
        if (val > best) {
          best = val;
          // IMPORTANT: keep reference to real piece, not the copy soit dosent desync
